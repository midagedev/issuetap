...
| Comment edit / delete | `PUT/DELETE /issue/{key}/comment/{id}` | Supported | PUT replaces the body only; `id`, `author`, `created`, `visibility` and `jsdPublic` stay, `updated` moves. A plain-string body is stored exactly as `POST …/comment` stores one (the Server dialect sends wiki markup, not ADF). DELETE is a real deletion — 204, and the comment is gone from the persist file on the next flush. An unknown comment id or issue key is HTTP 404 on both. |
...
| Writes | `POST /issue`, `PUT /issue/{key}`, `POST …/comment`, `PUT …/assignee`, `POST …/attachments`, `POST /issueLink` | Supported | Mutate the in-memory graph. Parent, when set, must exist and be exactly one `hierarchyLevel` above the child (type id, not name). Create 400: `errors.parent` and `errors.parentId`. Edit 400: `errors.pid`. An unknown project key on create is HTTP 400 `errors.project` — nothing is created, no implicit project bootstrap (Cloud semantics; projects come from the fixture, `POST /project`, or the persist file). `fields.issuetype` `{id}` or `{name}` must exist in the catalog (400 `errors.issuetype`); omitted defaults through the catalog — `10003` when present, else the lowest-id level-0 type — never a magic id a shadow eviction (GDK-1284) can dangle. On PUT, `priority`/`issuetype` unknown ids are HTTP 400 `errors.priority` / `errors.issuetype`, not silently stored dangling references. PUT is all-or-nothing: every field validates (a full probe pass) before anything is applied, so a mixed request with one invalid field applies nothing and records no changelog row; the first reported error is deterministic (update ops before fields, Cloud's order; then fields in sorted key order). POST and PUT `fields.fixVersions` / `fields.components` is a full replace (same resolver); `update.fixVersions` / `update.components` accepts `add`/`remove`/`set` with `{id}` or `{name}`. Identity is the project's issue-derived catalog (fixtures have no project version/component list). Unknown id/name is HTTP 400 `errors.fixVersions` / `errors.components`. Stored on the typed arrays, not `Custom`. A field id that is neither on the editable system set nor a registered custom field is HTTP 400 `errors.<field>` (Cloud's screen-or-unknown message) — never a silent `Custom` write. A project with an empty catalog accepts a name as-is. POST comment stores `visibility` (`type` must be role or group, `value` non-empty; no role/group existence check) and maps `properties` `sd.public.comment` `internal` to `jsdPublic` (`!internal`). Invalid `visibility.type` is HTTP 400 `errors.visibility`. POST `/issueLink` writes one element onto each issue, and each element labels the other end by that end's role (Cloud's convention: the issue POSTed as `outwardIssue` carries an `inwardIssue` element and vice versa — measured by `TestGadakLinkRoundTrip`: `gadak link TAP-1 TAP-3 --type blocks` POSTs `outwardIssue` TAP-3 / `inwardIssue` TAP-1, and then TAP-1 shows `outwardIssue` TAP-3 while TAP-3 shows `inwardIssue` TAP-1). Type is `{id}` or `{name}` against the GET catalog; unknown type is HTTP 404 `No issue link type …`. Missing issue is HTTP 404. Self-link is HTTP 400. Duplicate same type/pair/direction is idempotent 201 (no extra element). GET `issuelinks[].type` includes `id`/`inward`/`outward` from that catalog when the stored name matches. |
| Attachments | `GET /attachment/{id}`, `GET /attachment/content/{id}` | Supported | 302 to `/file/{uuid}/binary?name=`; the target serves the stored bytes with `Accept-Ranges`, `Content-Length` and an `ETag` (the content hash), so `Range` gets a 206 and a browser can seek. Upload (`POST …/attachments`) streams to disk and refuses past the configured cap with 413 — never a truncated 200. |
...
| CQL | `GET /wiki/rest/api/content/search` | Supported | `space`, `type`, `lastModified`; `ORDER BY lastmodified [asc|desc]` (asc default) is honored by the sort; an ORDER BY naming any other field is HTTP 400 — never a silent id-order fallback. `_links.next` percent-encodes the cql like Cloud's next links, so a client can follow it verbatim (raw concatenation corrupted any cql carrying spaces, quotes, `%` or `&`). |
...
(`lastmodified asc|desc` only; anything else is HTTP 400).

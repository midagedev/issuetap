...
| Dev status | `GET /rest/dev-status/{v}/issue/summary`, `GET /rest/dev-status/{v}/issue/detail`, `POST /rest/dev-status/{v}/issue/link` | Supported | Cloud internal development panel. `{v}` is `latest` or `1.0`. Summary counts stored pull-request links; other blocks are zero-valued. Detail requires `applicationType` and `dataType` (Cloud's 500 param shape). POST upserts one pull-request link by URL. Other `/rest/dev-status/*` subpaths are `unsupported_endpoint` (501). |
